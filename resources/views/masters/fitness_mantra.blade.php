...
<div class="row">
    ...
    <div class="col-md-8" style="margin-top: 2vh;">
        <form action="{{ route('fitness_mantra_store') }}" method="post" enctype="multipart/form-data">
            @csrf
        <table width="80%">
        ...
        <!-- START DEFAULT DATATABLE -->
             ...
             <div class="panel-body" style="margin-top:5px; margin-bottom:15px;">
                <table class="table datatable">
                    ...
                    <tbody>
                        @foreach ($fitness_mantra as $fitness_mantras)
                            
                        <tr>
                            <td>{{$loop->iteration}}</td>
                            <td>{{date('d-m-Y', strtotime($fitness_mantras->date))}}</td>
                            <td>{{$fitness_mantras->title}}</td>
                            <td>{{$fitness_mantras->author_name}}</td>
                            <td>
                                <a href="{{ asset('images/fitness_mantra_image/' . $fitness_mantras->image) }}" target="_blank">
                                <img src="{{ asset('images/fitness_mantra_image/' . $fitness_mantras->image) }}" width="50" height="50"/>
                                </a>
                            </td>
                          <td>{!! $fitness_mantras->content !!}</td>
                          
                         
                            <td>
                                <a href="{{ route('fitness_mantra_edit', $fitness_mantras->id) }}">
                                ...
                                <button onclick="openCustomModal('{{ route('fitness_mantra_destroy', $fitness_mantras->id) }}')"
